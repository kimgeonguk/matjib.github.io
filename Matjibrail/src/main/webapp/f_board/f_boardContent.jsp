...
request.setCharacterEncoding("utf-8");
	...
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
	   ...
	   F_BoardDAO dao = new F_BoardDAO();   
	  // int Recommand= dao.Recommand(Integer.parseInt(request.getParameter("mf_num"))); 
...
<div id="section">
	...
	<b style="font-size: 25pt; color: #81BEF7;">자유게시판</b>
	</div>
	<br><br><br>
	...
	<form action="">
		<table width="1300" border="0" cellpadding="0" cellspacing="0" align="center">
			<tr>
				<td colspan="2" align="center" width="50" style="text-align: left; font-size: 15pt; font-weight: bold;"><a style=" margin-left: 20%">No. <%=article.getMf_num() %></a></td>
				<td colspan="4" align="center" width="1220" style="font-size: 15pt; font-weight: bold"><%=article.getMf_subject() %></td>
			</tr>
			...
			<tr height="30">
				...
				<td align="center" width="120" style="margin-left: 10%; text-align: left;"  > <a style="margin-left: 20%;"><%=article.getMf_writer() %></a></td>
				<td width="80" style="text-align: right;"><a style="font-weight: bold;">작성일</a></td>
				<td colspan="2" align="center" width="250"><a style="margin-left: 20%; text-align: left;"><%=sdf.format(article.getMf_postdate()) %></a></td>
				<td width = "710" ></td>
			...
			<tr height="30"><td colspan="8" width="1300" style=" border-top: 1px solid #81BEF7;"></tr>
					...
					<input type="button" value="글쓰기" style="position:absolute; right:20%; width:70px; height:30px; background-color:#CEE3F6; border:1px solid; border-color:#81BEF7; border-radius: 5px; font-size:10pt; font-weight: bold; cursor: pointer;" 
							onClick="document.location.href='f_boardWriteForm.jsp'">
					&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<input type="button" value="수정" style="position:absolute; right:15%; width:70px; height:30px; background-color:#CEE3F6; border:1px solid; border-color:#81BEF7; border-radius: 5px; font-size:10pt; font-weight: bold; cursor: pointer;"
							onClick="document.location.href='f_boardUpdateForm.jsp?mf_num=<%=article.getMf_num()%>&pageNum=<%=pageNum%>'">
					&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<input type="button" value="삭제" style="position:absolute; right:10%; width:70px; height:30px; background-color:#CEE3F6; border:1px solid; border-color:#81BEF7; border-radius: 5px; font-size:10pt; font-weight: bold; cursor: pointer;"
							onClick="document.location.href='f_boardDeleteProc.jsp?mf_num=<%=article.getMf_num()%>&pageNum=<%=pageNum%>'">
					&nbsp;&nbsp;&nbsp;&nbsp;
...
count = dbPro.getArticleCount(mf_num);   // 전체 글 수

// 검색이 아니면 전체 리스트를 보여주고 검색이면 검색한 내ㅑ용만 보여줌

if(searchText == null) {
   count = dbPro.getArticleCount(mf_num);   // 전체 글수
   
   if(count > 0) {
      
      articleList = dbPro.getArticles(startRow, endRow, mf_num);
   }
}else {
...
<table width="1200" border="0" cellpadding="0" cellspacing="0" align="center">
   ...
   <tr height="30">
	...
	value="댓글삭제 "
	onClick="document.location.href='commentDeleteProc.jsp?mf_num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
	</td> 
...
if(count > 0) {
   ...
   if(startPage > pageBlock) {
      ...
      if(searchText == null) {
%>
      <a href="f_boardContent.jsp?mf_num=<%=mf_num %>&pageNum=<%=startPage-pageBlock%>">[이전]</a>
      <%
      }else {
      %>
      <a href="f_boardContent.jsp?mf_num=<%=mf_num %>&pageNum=<%=startPage-pageBlock%>&searchWhat=<%=searchWhat%>&searchText=<%=searchText%>"></a>
<%
      }
   }
   for(int i = startPage; i <= endPage; i++) {
      if(searchText == null) {
%>   
<a href="f_boardContent.jsp?mf_num=<%=mf_num %>&pageNum=<%=i%>">[<%=i%>]</a>
<%   }else { %>
   <a href="f_boardContent.jsp?mf_num=<%=mf_num %>&pageNum=<%=i%>&searchWhat=<%=searchWhat%>&searchText=<%=searchText%>"></a>
<% }
   }
   if(endPage < pageCount) {
      if(searchText == null) {
%>
<a href="f_boardContent.jsp?mf_num=<%=mf_num %>&pageNum=<%=startPage+pageBlock%>">[다음]</a>
<%}else { %>
   <a href="f_boardContent.jsp?mf_num=<%=mf_num %>&pageNum=<%=startPage+pageBlock%>&searchWhat=<%=searchWhat%>&searchText=<%=searchText%>"></a>
		...
		<form method="post" action="commentproc.jsp" >
			...
			<table class="table table-striped" width="1100" style="text-align: center; border: 0px solid #dddddd" >
				...
				<tr>
                    ...
                    <td><input type="hidden" name="c_num" value="<%=mf_num%>"></td> <!-- 게시번호 -->
